var app = new Vue({
	...
	computed: {
		...
		leftEyelidComputed() {
			if (this.isMouseDown) {
				return {'--left-eyelid-length-': `${this.leftMove.y*100/84}%`}
			} else {
		...
		rightEyelidComputed() {
			if (this.isMouseDown) {
				return {'--right-eyelid-length-': `${this.rightMove.y*100/84}%`}
			} else {
				return {transform: `translateY(${this.rightMove.y*100/84}%)`}
			}
		},
		leftEyelidClickComputed() {
			return this.isMouseDown ? 'left-eyelid-move' : '';
		},
		rightEyelidClickComputed() {
			return this.isMouseDown ? 'right-eyelid-move' : '';
		},
	},
	watch: {
		isMouseDown(newVal, oldVal) {
			if (newVal && !oldVal) {
				setTimeout(() => {
	...
	methods: {
		...
		mousemove(ev) {
			...
			let {
				offsetWidth: leftEyeW,
				offsetHeight: leftEyeH
			} = this.$refs.left_eye, {
				offsetWidth: rightEyeW,
				offsetHeight: rightEyeH
			} = this.$refs.right_eye, {
				left: leftEyeX,
				top: leftEyeY,
			} = this.$refs.left_eye.getBoundingClientRect(), {
				left: rightEyeX,
				top: rightEyeY
			} = this.$refs.right_eye.getBoundingClientRect();

			//left_eye_center / right_eye_center 圆心坐标
			let left_eye_center = {
					x: leftEyeX + leftEyeW / 2,
					y: leftEyeY + leftEyeH / 2
				},
				right_eye_center = {
					x: rightEyeX + rightEyeW / 2,
					y: rightEyeY + rightEyeH / 2
				};
